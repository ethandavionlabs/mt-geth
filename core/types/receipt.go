...
type receiptMarshaling struct {
	...
	// Mantle: extend receipts with their L1 price (if a rollup tx)
...
// LegacyMantleStoredReceiptRLP is the pre bedrock storage encoding of a
...
// entries.
type LegacyMantleStoredReceiptRLP struct {
	PostStateOrStatus []byte
	CumulativeGasUsed uint64
	Logs              []*LogForStorage
	L1GasUsed         *big.Int
...
func (r *ReceiptForStorage) DecodeRLP(s *rlp.Stream) error {
	...
	// First try to decode the latest receipt database format, try the pre-bedrock Mantle legacy format otherwise.
	if err := decodeStoredReceiptRLP(r, blob); err == nil {
		return nil
	}
	return decodeLegacyMantleReceiptRLP(r, blob)
}

func decodeLegacyMantleReceiptRLP(r *ReceiptForStorage, blob []byte) error {
	var stored LegacyMantleStoredReceiptRLP
...
func (rs Receipts) DeriveFields(config *params.ChainConfig, hash common.Hash, number uint64, time uint64, txs Transactions) error {
	...
	if config.Mantle != nil && len(txs) >= 2 { // need at least an info tx and a non-info tx
