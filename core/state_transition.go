...
func (st *StateTransition) preCheck() error {
	if st.msg.IsDepositTx() {
		...
		if st.msg.IsSystemTx() {
			if st.evm.ChainConfig().IsMantleRegolith(st.evm.Context.Time) {
...
func (st *StateTransition) innerTransitionDb() (*ExecutionResult, error) {
	...
	if st.msg.IsDepositTx() && !rules.IsMantleRegolith {
		// Record deposits as using all their gas (matches the gas pool)
		// System Transactions are special & are not recorded as using any gas (anywhere)
		gasUsed := st.msg.Gas()
		if st.msg.IsSystemTx() {
	...
	// Check that we are post bedrock to enable mt-geth to be able to create pseudo pre-bedrock blocks (these are pre-bedrock, but don't follow l2 geth rules)
	// Note mantleConfig will not be nil if rules.IsMantleBedrock is true
	if mantleConfig := st.evm.ChainConfig().Mantle; mantleConfig != nil && rules.IsMantleBedrock {
		st.state.AddBalance(params.MantleBaseFeeRecipient, new(big.Int).Mul(new(big.Int).SetUint64(st.gasUsed()), st.evm.Context.BaseFee))
		if cost := st.evm.Context.L1CostFunc(st.evm.Context.BlockNumber.Uint64(), st.evm.Context.Time, st.msg); cost != nil {
			st.state.AddBalance(params.MantleL1FeeRecipient, cost)
		}
	}
