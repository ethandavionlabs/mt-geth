...
func (w *worker) makeEnv(parent *types.Block, header *types.Header, coinbase common.Address) (*environment, error) {
	...
	if err != nil && w.chainConfig.Mantle != nil { // Allow the miner to reorg its own chain arbitrarily deep
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	// Set the extra field.
	if len(w.extra) != 0 && w.chainConfig.Mantle == nil { // Mantle chains must not set any extra data.
	...
	} else if w.chain.Config().Mantle != nil && w.config.GasCeil != 0 {
		// configure the gas limit of pending blocks with the miner gas limit config when using mantle
